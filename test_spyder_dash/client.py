...
def draw(screen, background_image, heart_image, l_player,l_prop, l_particles, l_skin, l_laser, screen_h, screen_w, l_brick):
    """
...
class Player:
  def __init__(self, id, x, y, w, h, skin_num, hp, score, s_x, s_y, l_laser, l_bullet, skin_name, addr):
    ...
    if(skin_name in l_skin):
        self.skin = pygame.image.load("./skin/"+skin_name).convert_alpha()
    else:
...
l_player.append([200, 400, 50, 50, 0, 0, 9.81, 0, 0, 0, True, 0, True, 50, 0])
...
while running_spider_dash:
    ...
    draw(screen, background_image, heart_image, l_player,l_prop, l_particles, l_skin, l_laser, screen_h, screen_w, l_brick)
    l_laser_player = []
...
cap = cv2.VideoCapture("./skin/factory_skin/dead.mp4")
if (cap.isOpened()== False):  
    print("Error")
while(cap.isOpened()): 
    ret, frame = cap.read() 
    if ret == True: 
        cv2.imshow('You are dead', frame) 
