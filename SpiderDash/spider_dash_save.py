...
l_prop = [[-100, screen_h-40, screen_w+200, 100], [-121, -100, 100, screen_h+150], [screen_w+21, -100, 100, screen_h+150], [-100, -100, screen_w+200, 100],[243, 214, 349, 59], [478, 435, 382, 88], [1070, 314, 439, 75]]
l_enemy = []
...
def draw(screen, background_image, l_player, l_enemy,l_prop, l_particles, l_skin):
    """
...
#[x, y, w, h, vx, vy, gravity, is_on_ground, spider_x, spider_y, apply_colision, skin_number]
...
l_player.append([200, 100, 50, 50, 0, 0, 9.81, 0, 0, 0, True, 0])
l_skin = list_of_files("./skin", "")
...
for i in range(3):
    l_enemy.append([random.randint(200,1400), 1500, 50, 50, 0, 0, 9.81, 0, 0, random.randint(7,14), True])
    l_enemy.append([random.randint(200,1400), 80, 50, 50, 0, 0, 9.81, 0, 1, random.randint(3,6), True])
    l_enemy.append([random.randint(200,1400), 80, 50, 50, 0, 0, 9.81, 0, 0, random.randint(7,14), True])
    l_enemy.append([random.randint(200,1400), 80, 50, 50, 0, 0, 0, 0, 2, random.randint(3,4), False])
...
while in_menu_spider_dash:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.VIDEORESIZE:
            ...
            background_image = pygame.transform.scale(background_image, (screen_w, screen_h+150))
            l_prop[0], l_prop[1], l_prop[2], l_prop[3]= ([-100, h-40, w+200, 100], [-121, -100, 100, h+150], [w+21, -100, 100, h+150], [-100, -100, w+200, 100])
...
while running_spider_dash:
    keys = pygame.key.get_pressed()
    draw(screen, background_image, l_player, l_enemy, l_prop, l_particles, l_skin)
    l_to_remove = []
    for i, enemy in enumerate(l_enemy):
        x = enemy[0]
        y = enemy[1]
        ...
        for bullet in l_bullet:
            ...
            if rect.clipline((b[0],b[1]), (b[0]+b[2]*20,b[1]+b[3]*20)):
                ...
                for i in range(20):
                    l_particles.append([random.randint(-w,w)+x+w/2, random.randint(-h,h)+y+h/2, random.uniform(0.05, 0.4), random.choice(["red","orange","orange","yellow", "yellow", "yellow"])])
